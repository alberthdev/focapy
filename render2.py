...
SRC_FILE          = "read_diag.f90"
MODULE            = "read_diag"

NEW_SRC_FILE      = "focapy_out/read_diag/read_diag.f90"
NEW_HEADER_FILE   = "focapy_out/read_diag/read_diag.h"
NEW_SWIG_FILE     = "focapy_out/read_diag/read_diag.i"
NEW_FCWRAP_FILE   = "focapy_out/read_diag/fcwrap.f90"
...
# 
args_hinting_config = {}
args_hinting_config[MODULE] = {}
args_hinting_config[MODULE] = {}
args_hinting_config[MODULE]["read_array"] = {}
args_hinting_config[MODULE]["read_array"]["array"] = { 
                                                            "cue" : "ndata",
                                                            "type": "out",
                                                           }
args_hinting_config[MODULE]["print_array"] = {}
args_hinting_config[MODULE]["print_array"]["array"] = { 
                                                            "cue" : "ndata",
                                                            "type": "in",
                                                           }
args_hinting_config[MODULE]["ret_mean_from_array"] = {}
args_hinting_config[MODULE]["ret_mean_from_array"]["array"] = { 
                                                            "cue" : "ndata",
                                                            "type": "in",
                                                           }
...
print " * Preprocessing %s => %s..." % (SRC_FILE, NEW_SRC_FILE)
...
#############################################
for subroutine in module_dict[MODULE]["subroutines"]:
    print " * ARGS HINTING: Processing subroutine:", subroutine
    subroutine_dict = module_dict[MODULE]["subroutines"][subroutine]
        ...
        for subroutine_arg in subroutine_dict["arguments"]:
            ...
            if ("dimension(:)" in subroutine_arg_dict['attributes']):
                ...
                if not (("intent(in)" in subroutine_arg_dict['attributes']) or ("intent(inout)" in subroutine_arg_dict['attributes']) or ("intent(out)" in subroutine_arg_dict['attributes'])):
                    print "ERROR: Unknown or no intent specified! (Arg: %s, Attr: %s)" % (subroutine_arg, ", ".join(subroutine_arg_dict['attributes']))
                    sys.exit(1)
                
                hint_complete = False
                
                if MODULE in args_hinting_config:
                    if not MODULE in args_hinting:
                        args_hinting[MODULE] = {}
                    if not subroutine in args_hinting[MODULE]:
                        args_hinting[MODULE][subroutine] = {}
                    if (subroutine in args_hinting_config[MODULE]) and (subroutine_arg in args_hinting_config[MODULE][subroutine]):
                        if not subroutine_arg in args_hinting[MODULE]:
                            args_hinting[MODULE][subroutine][subroutine_arg] = {}
                        arg_hinting_dict = args_hinting_config[MODULE][subroutine][subroutine_arg]
                        ...
                        if ("intent(in)" in subroutine_arg_dict['attributes']):
                            if arg_hinting_dict["type"] == "out":
                                ...
                                sys.exit(1)
                            args_hinting[MODULE][subroutine][subroutine_arg]["type"] = "in"
                            ...
                            if (arg_hinting_dict["type"] == "in") or (arg_hinting_dict["type"] == "inout"):
                                ...
                                args_hinting[MODULE][subroutine][subroutine_arg]["type"] = "out"
                        ...
                        if args_hinting_config[MODULE][subroutine][subroutine_arg]["cue"]:
                            args_hinting[MODULE][subroutine][subroutine_arg]["cue"] = args_hinting_config[MODULE][subroutine][subroutine_arg]["cue"]
                        else:
                            args_hinting[MODULE][subroutine][subroutine_arg]["cue"] = None
...
for function in module_dict[MODULE]["functions"]:
    print " * ARGS HINTING: Processing function:", function
    function_dict = module_dict[MODULE]["functions"][function]
        ...
        for function_arg in function_dict["arguments"]:
            ...
            if ("dimension(:)" in function_arg_dict['attributes']):
                ...
                if MODULE in args_hinting_config:
                    if not MODULE in args_hinting:
                        args_hinting[MODULE] = {}
                    if not function in args_hinting[MODULE]:
                        args_hinting[MODULE][function] = {}
                    if (function in args_hinting_config[MODULE]) and (function_arg in args_hinting_config[MODULE][function]):
                        if not function_arg in args_hinting[MODULE]:
                            args_hinting[MODULE][function][function_arg] = {}
                        arg_hinting_dict = args_hinting_config[MODULE][function][function_arg]
                        if not (("cue" in arg_hinting_dict) and ("type" in arg_hinting_dict)):
                            print "ERROR: Args hinting configuration must have 'cue' and 'type' field!"
                            sys.exit(1)
                        ...
                        if ("intent(in)" in function_arg_dict['attributes']):
                            if arg_hinting_dict["type"] == "out":
                                print "ERROR: Original intent 'in' can't be changed to an 'out'!"
                                sys.exit(1)
                            args_hinting[MODULE][function][function_arg]["type"] = "in"
                        elif ("intent(inout)" in function_arg_dict['attributes']):
                            if (arg_hinting_dict["type"] == "in") or (arg_hinting_dict["type"] == "inout"):
                                args_hinting[MODULE][function][function_arg]["type"] = "in"
                            elif arg_hinting_dict["type"] == "out":
                                args_hinting[MODULE][function][function_arg]["type"] = "out"
                        ...
                        if args_hinting_config[MODULE][function][function_arg]["cue"]:
                            args_hinting[MODULE][function][function_arg]["cue"] = args_hinting_config[MODULE][function][function_arg]["cue"]
                        else:
                            args_hinting[MODULE][function][function_arg]["cue"] = None
...
if len(module_dict[MODULE]["elements"].keys()) > 0:
    for element in module_dict[MODULE]["elements"]:
        print " * Processing top-level element:", element
        element_dict = module_dict[MODULE]["elements"][element]
        elements_conv_dict[element] = cdef.make_type(element_dict['type'], element)

derived_types_conv_dict = OrderedDict()

# TODO: handle weird attributes?
for d_type in module_dict[MODULE]["types"]:
    print " * Processing derived type:", d_type
    d_type_dict = module_dict[MODULE]["types"][d_type]
...
for subroutine in wrap_subroutines:
    ...
    if len(subroutine_dict["arguments"].keys()) > 0:
        for subroutine_arg in subroutine_dict["arguments"]:
            ...
            #print cdef.make_type(subroutine_arg_dict['type'], subroutine_arg)
            ...
            if ("dimension(:)" in subroutine_arg_dict['attributes']):
                #print subroutine_arg_dict['attributes']
...
output_from_header_template = header_template.render(module_name = MODULE, \
...
if len(double_pointer_types) > 0:
    output_from_swig_template = swig_template.render(module_name = MODULE, \
...
output_from_fcwrap_template = fcwrap_template.render(module_name = MODULE, \
                                wrap_subroutines = wrap_subroutines, \
                                wrap_functions = wrap_functions, \
                                subroutines = module_dict[MODULE]["subroutines"], \
                                functions = module_dict[MODULE]["functions"],
