program test_dummy
  ...
  use read_dummy, only: header_list, data_list, get_header, get_data, mean_type, &
                        stddev_type, inc_mean, ret_mean, ret_mean_from_array_focapy_orig, init_dummy, &
                        set_ndata, print_array_focapy_orig, read_array_focapy_orig, write_array_focapy_orig
  ...
  call print_array_focapy_orig(cur_array,head%ndata)

  print *,'Mean from mean_type:           ',ret_mean(cur_mean) ! return mean from sequential calculation
  print *,'Mean from ret_mean_from_array: ',ret_mean_from_array_focapy_orig(cur_array,head%ndata) !return mean from instantaneous array calc

  lun = 123
  fn  = 'test.output.binary'

  call write_array_focapy_orig(lun,fn,cur_array,head%ndata)            ! write array to 'self-describing' binary file (first element ndata, second element array(ndata) )
  call read_array_focapy_orig(lun,fn,array_from_file,ndata_from_array) ! read array from 'self-describing' binary file

  call print_array_focapy_orig(array_from_file,ndata_from_array)
