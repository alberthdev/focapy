...
int main() {
    ...
    print_array(&cur_array,&(head->ndata));
    ...
    printf(" Mean from ret_mean_from_array:  %10.6f\n",ret_mean_from_array(&cur_array,&(head->ndata))); //return mean from instantaneous array calc
    ...
    read_array(&lun,fn,&array_from_file,&ndata_from_file); // read array from 'self-describing' binary file
    
    print_array(&array_from_file,&ndata_from_file);
}
