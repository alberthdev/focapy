                ...
                for reg in custom_types[type_base]["regex"]:
                    ...
                    if match:
                        ...
                        if len(custom_types[type_base]["map"]) != len(match.groups()):
                            print "ERROR: Number of regex matches does not match number of maps! (Map has %i, match has %i)" % (len(custom_types[type_base]["map"]), len(match.groups()))
                            sys.exit(1)
                        map_dict = {}
        ...
        for type in mod.types:
            ...
            if len(type.elements) > 0:
                for type_ele in type.elements:
                    ...
                    elif type_ele_t_base in custom_types.keys():
                        # Validation
                        if "regex" not in custom_types[type_ele_t_base]:
                            print "ERROR: Custom types dict is invalid! (Regex definition not found!)"
                            sys.exit(1)
                        if "map" not in custom_types[type_ele_t_base]:
                            print "ERROR: Custom types dict is invalid! (Map definition not found!)"
                            sys.exit(1)
                        if "format" not in custom_types[type_ele_t_base]:
                            print "ERROR: Custom types dict is invalid! (Map definition not found!)"
                            sys.exit(1)
                        for reg in custom_types[type_ele_t_base]["regex"]:
                            regex = re.compile(reg)
                            match = regex.search(type_ele_t)
                            if match:
                                # Map check
                                if len(match.groups()) == 0:
                                    print "ERROR: No match groups defined for regex!"
                                    sys.exit(1)
                                if len(custom_types[type_ele_t_base]["map"]) != len(match.groups()):
                                    print "ERROR: Number of regex matches does not match number of maps! (Map has %i, match has %i)" % (len(custom_types[type_base]["map"]), len(match.groups()))
                                    sys.exit(1)
                                map_dict = {}
                                map_res = match.groups()
                                for i in xrange(0, len(map_res)):
                                    # Validate
                                    map_split = custom_types[type_ele_t_base]["map"][i].split("=")
                                    if len(map_split) != 2:
                                        print "ERROR: Map specification is invalid! It should be variable=%TYPE%!"
                                        sys.exit(1)
                                    if not ((map_split[1][0] == "%") and (map_split[1][-1] == "%")):
                                    ...
                                    if map_type == "%INT%":
                                        ...
                                        print "ERROR: Map specification is invalid! (Invalid type found - valid: %INT%, %FLOAT%, %STRING%)"
                                        sys.exit(1)
                        final_format = custom_types[type_ele_t_base]["format"]
                        # Variable replacement
                        for var in map_dict.keys():
                            final_format = final_format.replace("%%" + var + "%%", str(map_dict[var]))
                        # Static variable replacement
                        final_format = final_format.replace("%NAME%", type_ele.name);
                        final_format = final_format.replace("%POINTER%", "*");
                ...
                for sub_arg in subroutine.arguments:
                    ...
                    if type in default_types:
                        ...
                        for var in map_dict.keys():
                            final_format = final_format.replace("%%" + var + "%%", str(map_dict[var]))
                        # Static variable replacement
                        final_format = final_format.replace("%NAME%", sub_arg.name);
                        final_format = final_format.replace("%POINTER%", "*");
                        
                        # Write it out!
                        args_list.append(final_format)
                        args_map[type] = final_format
                    else:
                        print "WARNING: Could not find equivalent C definition for type '%s'!" % type
                        print "Make sure you define it!"
                        header_fh.write("/* STUB: %s %s,*/ " % (type, sub_arg.name))
